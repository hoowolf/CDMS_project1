...
class TestReceive:
    ...
    def pre_run_initialization(self):
        ...
        # 发货订单
        self.seller = Seller(conf.URL, self.seller_id, self.seller_password)
    ...
    def test_error_user_id(self):
        # 错误的用户ID（非买家）
        wrong_buyer = register_new_buyer(self.buyer_id + "_x", self.buyer_password)
        code = wrong_buyer.receive(self.order_id)
        assert code != 200
    ...
    def test_error_non_sent_order(self):
        # 测试未发货的订单无法收货（需要重新创建一个已付款但未发货的订单）
